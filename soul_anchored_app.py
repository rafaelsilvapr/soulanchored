...
try:
    ...
    try:
        if "SUPABASE_URL" not in st.secrets:
            st.error("⚠️ Configuração 'SUPABASE_URL' ausente nos Secrets.")
            st.stop()
    ...
    with tab1:
        ...
        with col2:
            audio_file_uploader = st.file_uploader("Upload de Áudio da Narração (.mp3/wav)", type=['mp3', 'wav'])
            if audio_file_uploader:
                st.audio(audio_file_uploader)

        if st.button("🧠 Gerar Storyboard Baseado no Áudio"):
            if not script_text or not audio_file_uploader:
                st.warning("Forneça o roteiro e o áudio para continuar.")
            else:
                with tempfile.NamedTemporaryFile(delete=False, suffix=f".{audio_file_uploader.name.split('.')[-1]}") as tmp:
                    tmp.write(audio_file_uploader.getvalue())
                    tmp_path = tmp.name
